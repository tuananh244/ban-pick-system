...
// --- CÁC HÀM HELPER NỘI BỘ (PURE FUNCTIONS) ---
...
const getPerfScore = (val: number, useAV: boolean) => {
...
export const registerAdminHandlers = (io: Server, socket: Socket, roomId: string, role: string) => {
    ...
    // Hàm bọc logic kiểm tra Admin và lấy Room
    const getRoomAsAdmin = async () => {
        if (role !== 'admin') return null;
        return await roomStore.get(roomId);
    };

    // 1. Mở khóa trạng thái Ready cho Player chỉnh sửa lại
    socket.on("admin_unlock_players", async () => {
        const r = await getRoomAsAdmin();
        if (!r) return;
        r.p1Ready = false;
        r.p2Ready = false;
    ...
    socket.on("admin_init_fill_phase", async ({ teamCount }) => {
        const r = await getRoomAsAdmin();
        if (!r) return;
        ...
        const defaultConfig = { 
            turns: 5, av: 0, isCleared: true, hasCastoriceTalent: false, deadCount: 0 
        };
        r.p1TeamConfigs = Array.from({ length: teamCount }, () => ({ ...defaultConfig }));
        r.p2TeamConfigs = Array.from({ length: teamCount }, () => ({ ...defaultConfig }));
    ...
    // 3. Thêm nhân vật vào slot trống (ADMIN)
    socket.on("admin_fill_add", async ({ teamSide, teamIndex, char }) => {
        const r = await getRoomAsAdmin();
        if (!r) return;
        ...
        teams[teamIndex][emptySlotIndex] = { 
            ...char, eidolon: 0, equippedWeapon: null, weaponRank: 1, isFilled: true 
        };
    ...
    // 4. Xóa nhân vật khỏi slot
    socket.on("admin_fill_remove", async ({ teamSide, teamIndex, charIndex }) => {
        const r = await getRoomAsAdmin();
        if (!r) return;
        const teams = teamSide === 'p1' ? r.p1FinalTeams : r.p2FinalTeams;
        if (teams[teamIndex]) {
    ...
    // 5. Đổi chỗ nhân vật (Swap)
    socket.on("admin_fill_swap", async ({ teamSide, teamIndex1, slotIndex1, teamIndex2, slotIndex2 }) => {
        const r = await getRoomAsAdmin();
        if (!r) return;
        const teams = teamSide === 'p1' ? r.p1FinalTeams : r.p2FinalTeams;
    ...
    // 6. Cập nhật chỉ số E/S cho nhân vật
    socket.on("admin_update_fill_stats", async ({ teamSide, teamIndex, charIndex, updates }) => {
        const r = await getRoomAsAdmin();
        if (!r) return;
        const teams = teamSide === 'p1' ? r.p1FinalTeams : r.p2FinalTeams;
    ...
    // 7. Cập nhật kết quả trận đấu (Turns/AV/Dead)
    socket.on("admin_update_team_config", async ({ teamSide, teamIndex, config }) => {
        const r = await getRoomAsAdmin();
        if (!r) return;
    ...
    socket.on("admin_calc_score", async ({ logicMode, useAV }) => {
        const r = await getRoomAsAdmin();
        if (!r) return;
        ...
        for (let i = 0; i < teamCount; i++) {
            ...
            const perf1 = getPerfScore(useAV ? c1.av : c1.turns, useAV);
            const perf2 = getPerfScore(useAV ? c2.av : c2.turns, useAV);
            ...
            if (logicMode === 1) { // Mode 1: Ưu tiên Cleared
    ...
    // 9. Xóa phòng
    socket.on("admin_terminate", async () => {
        if (role === 'admin') {
            console.log(`⚠️ Terminating Room: ${roomId}`);
            
            // Phát tín hiệu đuổi player ra trước
            io.to(roomId).emit("terminated");

            // Gọi hàm xóa và phải có AWAIT
