...
const ConfigModal: React.FC<Props> = ({ mode, char, weapons, bannedWeaponIds, onClose, onUpdate }) => {
  // --- 1. LOCAL STATE ĐỂ TRÁNH RE-RENDER GÂY ĐÓNG MODAL ---
  const [localChar, setLocalChar] = useState(char);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedPath, setSelectedPath] = useState<string | null>(null);
  const [selectedRarity, setSelectedRarity] = useState<number | null>(null);

  // Cập nhật localChar nếu char prop thay đổi (đảm bảo dữ liệu luôn mới nhất khi mở)
  useEffect(() => {
    setLocalChar(char);
    if (char?.path) {
      const pathEn = translatePathToEn(char.path);
      setSelectedPath(PATHS.includes(pathEn) ? pathEn : null);
    }
  }, [char]);

  // Hàm cập nhật tạm thời vào state nội bộ
  const updateLocal = (fields: any) => {
    setLocalChar((prev: any) => ({ ...prev, ...fields }));
  };

  // Hàm gửi dữ liệu cuối cùng lên Server và đóng Modal
  const handleConfirm = () => {
    onUpdate(localChar);
    onClose();
  };

  // --- THEME HELPER GIỮ NGUYÊN ---
  const getTheme = (rarity: number) => {
    switch (rarity) {
      case 5: return { border: 'border-[#d4af37]', shadow: 'shadow-[0_0_15px_rgba(255,215,0,0.4)]', text: 'text-[#ffd700]', bg: 'bg-[#1f1605]' };
      case 4: return { border: 'border-[#7c3aed]', shadow: 'shadow-[0_0_15px_rgba(124,58,237,0.4)]', text: 'text-[#c4b5fd]', bg: 'bg-[#110e1c]' };
      default: return { border: 'border-[#0ea5e9]', shadow: 'shadow-[0_0_15px_rgba(14,165,233,0.4)]', text: 'text-[#7dd3fc]', bg: 'bg-[#0c1620]' };
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm animate-fade-in" onClick={onClose}>
      ...
      >
        ...
        {/* NÚT XÁC NHẬN CHỐT DỮ LIỆU */}
        <div className="absolute bottom-6 right-6 z-50 flex gap-3">
            <button onClick={handleConfirm} className="px-8 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl font-black uppercase shadow-lg shadow-cyan-900/20 transition-all hover:scale-105 active:scale-95">XÁC NHẬN</button>
        </div>
        ...
        {/* LEFT COLUMN: EIDOLON (Sử dụng localChar) */}
        <div className="w-20 bg-[#080a0f] border-r border-white/5 flex flex-col items-center py-4 shrink-0 z-10 relative">
          ...
          <div className="flex-1 flex flex-col justify-end gap-2 w-full px-2 pb-20"> {/* Tăng pb để tránh bị nút Xác nhận đè */}
            {[6, 5, 4, 3, 2, 1, 0].map(lvl => {
               const isActive = (localChar.eidolon || 0) === lvl;
               return (
                <button
                  key={lvl}
                  onClick={() => updateLocal({ eidolon: lvl })}
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-gradient-to-br from-[#11141b] to-[#0b0e14]">
          ...
          {/* HEADER GIỮ NGUYÊN CONFIG */}
          <div className="flex flex-col border-b border-white/5 bg-[#14171f] p-4 gap-4 shrink-0 shadow-lg z-20">
            <div className="flex items-end justify-between gap-4 mr-16"> 
               <div className="flex items-center gap-4 min-w-0 flex-1">
                 <div className="w-16 h-16 rounded-xl border border-white/20 overflow-hidden shadow-lg shrink-0 bg-black">
                    <img src={localChar.image} className="w-full h-full object-cover" alt={localChar.name} />
                 </div>
                 <div className="flex flex-col min-w-0">
                     <div className="flex items-center gap-2 mb-1 opacity-50">
                        <img src={`/images/path/${translatePathToEn(localChar.path)}.png`} className="w-5 h-5 object-contain brightness-0 invert" alt={localChar.path} />
                        <span className="text-[10px] font-bold uppercase tracking-widest text-white whitespace-nowrap">{translatePathToEn(localChar.path)}</span>
                     </div>
                     <h3 className="text-2xl font-black italic uppercase text-white tracking-tighter leading-none truncate">{localChar.name}</h3>
                 </div>
               </div>
            ...
            {/* EQUIPPED WEAPON INFO (Sử dụng localChar) */}
            {localChar.equippedWeapon && (
              <div className="flex items-center justify-between bg-black/20 rounded-lg p-2 border border-white/5 animate-in slide-in-from-top-2">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded border border-white/10 overflow-hidden shrink-0">
                       <img src={localChar.equippedWeapon.image} className="w-full h-full object-cover" alt="LC" />
                    </div>
                    <div className="flex flex-col leading-none">
                       <span className="text-[8px] font-bold text-gray-500 uppercase">EQUIPPED</span>
                       <span className={`text-xs font-black uppercase truncate max-w-[200px] ${getTheme(localChar.equippedWeapon.rarity).text}`}>
                          {localChar.equippedWeapon.name}
                       </span>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2 items-center mr-16">
                    ...
                    <div className="flex gap-1">
                       {[1, 2, 3, 4, 5].map(rank => (
                          <button key={rank} onClick={() => updateLocal({ weaponRank: rank })} className={`w-6 h-6 rounded flex items-center justify-center text-[10px] font-black transition-all border ${(localChar.weaponRank || 1) === rank ? 'bg-amber-500 border-amber-400 text-black shadow-[0_0_10px_rgba(245,158,11,0.5)]' : 'bg-transparent border-white/10 text-gray-600 hover:text-white hover:border-white/30'}`}>{rank}</button>
                       ))}
                    </div>
                    <div className="w-[1px] h-4 bg-white/10 mx-1"></div>
                    <button onClick={() => updateLocal({ equippedWeapon: null, weaponRank: 1 })} className="w-6 h-6 rounded bg-red-500/10 border border-red-500/30 text-red-500 hover:bg-red-500 hover:text-white flex items-center justify-center transition-all">✕</button>
                  </div>
              </div>
            )}
          </div>

          {/* WEAPON GRID GIỮ NGUYÊN CSS */}
          <div className="flex-1 p-4 overflow-y-auto custom-scrollbar pb-24">
              <div className="grid grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3">
                  {filteredWeapons.map((wpn) => {
                    const isEquipped = localChar.equippedWeapon?.id === wpn.id;
                    const theme = getTheme(wpn.rarity);

                    return (
                      <button
                        key={wpn.id}
                        onClick={() => updateLocal({ equippedWeapon: wpn, weaponRank: 1 })}
                        className={`
                        ...
                        `}
                      >
                          <img src={wpn.image} className="w-full h-full object-cover" alt={wpn.name} />
