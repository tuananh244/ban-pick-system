...
// --- COMPONENT SOÁT ĐIỂM (HIỂN THỊ CHI TIẾT STATS LẺ TỪ FIREBASE) ---
const ScoreDebugger = ({ p1Teams, p2Teams, p1Configs, p2Configs }: any) => {
            ...
            <div className={`flex-1 flex flex-col gap-4 p-5 rounded-3xl border ${borderColor} ${bgColor} backdrop-blur-md`}>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4 pr-1">
                    {teams.map((team, tIdx) => {
                        ...
                        // LOGIC: Nếu FAILED thì Talent và Death coi như bằng 0
                        const talentPoints = (config.isCleared && config.hasCastoriceTalent) ? 2 : 0;
                        const deathPenalty = config.isCleared ? (Number(config.deadCount) || 0) * 3 : 0;
                        
                        return (
                            <div key={tIdx} className="bg-black/60 rounded-2xl border border-white/10 overflow-hidden shadow-2xl">
                                <div className="bg-white/10 px-4 py-2 flex justify-between items-center">
                                    ...
                                    <div className="flex gap-2">
                                        {talentPoints > 0 && <span className="text-[10px] font-black bg-pink-500/20 text-pink-400 px-2 py-0.5 rounded">T+2</span>}
                                        {deathPenalty > 0 && <span className="text-[10px] font-black bg-red-500/20 text-red-400 px-2 py-0.5 rounded">D+{deathPenalty}</span>}
                                    </div>
                                </div>
                                <div className="p-3">
                                                    ...
                                                    <tr key={cIdx}>
                                                        <td className="py-3 flex items-center gap-3">
                                                            ...
                                                            <div className="min-w-0">
                                                                ...
                                                                <div className="text-[10px] text-white/40 truncate italic"><span className="text-orange-400 font-bold">S{char.weaponRank}</span> {char.equippedWeapon?.name || "None"}</div>
                                                            </div>
                                                        </td>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in duration-500">
            <div className="w-full max-w-5xl bg-[#0b0e14] border border-white/10 rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col">
                ...
                <div className="p-8 bg-black/40 overflow-y-auto custom-scrollbar flex-1">
                    {showDebugger ? (
                        <ScoreDebugger p1Teams={p1Teams} p2Teams={p2Teams} p1Configs={p1Configs} p2Configs={p2Configs} />
                    ) : (
